...
- Local development and phase validation: Docker Postgres via
  `docker compose up -d postgres`.
- Shared preview, staging, and production: Neon Postgres by default.

Why Neon remotely: it is managed Postgres, supports pgvector, and database
branches map cleanly to our cumulative Git phase branches (`phase/0-*`,
`phase/1-*`, etc.). If you validate an old phase from its branch, use a matching
Neon branch/database or a disposable local DB so migrations from another phase
do not leak into the check.

Alternatives:

- Supabase when the product intentionally needs Supabase Auth, Storage,
  Realtime, or its dashboard.
- Render/Railway when deployment simplicity matters more than DB branching.
- Native local Postgres is allowed, but Docker is preferred on Windows because
  pgvector setup is more reproducible.

Use `pnpm db:migrate` for schema changes. For Neon, use the direct connection
string for migrations; add a separate pooled runtime URL only after the app
actually needs runtime pooling.
