...
- `apps/miniapp/` — Vite + React 18 + Telegram SDK Mini App
  ...
  - `src/telegram/` — WebApp boot, theme listener, AppRoot, BackButton/MainButton hooks; `openLink.ts` routes external URLs via `WebApp.openLink(try_instant_view:true)` with a `window.open` fallback
...
- `packages/db/` — Drizzle ORM + Postgres pool + migrations
  ...
  - `migrations/0009_phase5_perf_indexes.sql` / `.down.sql` — Phase 5 perf: NULLS LAST on radar listing index + partial topic_profiles pending-embedding index
  - `migrations/0010_phase5_extend_radar_index.sql` / `.down.sql` — Phase 5 perf: extend radar listing index with `created_at DESC` to satisfy listRadarMatches ORDER BY tie-breaker directly
...
- `architecture/matching-and-scoring.md` — Phase 5 per-workspace radar. *Active. Latest closure `phase-5-perfect-r5` MIN=10 PERFECT.* 2 new DB tables (`workspace_news_matches`, `ai_usage_events`), 3 new task types (`match_news_to_workspaces`, `score_workspace_match`, `recompute_topic_embedding`), real `AIProvider.score()` on Yandex DeepSeek 3.2 (completion + zod + repair-attempt + TemplateProvider fallback), composite score (LLM 50% + cosine 30% + freshness 10% + reliability 10%), `GET /radar` route, Mini App Radar screen with ARIA APG tablist (arrow-key nav + roving tabindex + :focus-visible) and Russian plural agreement. Loop-iterated migrations 0009 (NULLS LAST + topic_profiles partial index) and 0010 (extended radar index with created_at DESC). Cost guard remains STUB (Phase 6 hook, named owner in roadmap). Step-perfect-loop closure: PERFECT after Phase Closure Discipline / Production Readiness Gates mapped prior follow-ups to named Phase 6/7/8 owners.

## Recent changes (last 10)

- 2026-05-17: Phase 5 r5 fresh step-perfect-loop validation closed on `phase/5-matching-scoring` with tag `phase-5-perfect-r5`. Found local unpushed r4 closure (`phase-5-perfect-r4`, MIN=10 PERFECT) already present; r5 fixed the remaining PROJECT_MAP active-system drift from r3 to r5 and revalidated the r4/r5 artifact. Gates passed: `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `SKIP_DB_TESTS=1 pnpm test` (first full run hit two 5s API timeouts under load; both targeted tests passed; repeat full run passed with 593 passed, 21 skipped), `pnpm build`, Mini App bundle budget (168876 B, +8.02% vs baseline, under 10%), `.codex/kit/diagnose.ps1`, `.claude/kit/diagnose.ps1`, and `git diff --check`. Runtime code unchanged after r2; r4/r5 are closure/rules/docs synchronization.

- 2026-05-17: Phase 5 r3 fresh step-perfect-loop validation closed on `phase/5-matching-scoring` with tag `phase-5-perfect-r3`. No runtime code changes were required after r2. Gates passed: `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `SKIP_DB_TESTS=1 pnpm test` (first full run hit two 5s API timeouts under load; both targeted tests passed; repeat full run passed with 593 passed, 21 skipped), `pnpm build`, Mini App bundle budget (168876 B, +8.02% vs baseline, under 10%), `.codex/kit/diagnose.ps1`, `.claude/kit/diagnose.ps1`, and `git diff --check`. Final status remains 8/10 GOOD with UNREACHABLE_10 reason (a): remaining caps are live Neon/Yandex/e2e and Phase 6+/8 ops scope.

- 2026-05-17: Phase 5 r2 step-perfect-loop closed on `phase/5-matching-scoring` with tag `phase-5-perfect-r2`. Fresh pass found and fixed 3 practical r1 residuals: cross-item cluster dedup race in `upsertWorkspaceNewsMatch` (second advisory lock on `(workspace, cluster)` + existing-cluster short-circuit + stale item-row hide), silent per-workspace fan-out loss in `match_news_to_workspaces` (process all targets, then retry task if any target failed), and `ai_usage_events.error_message` over-500 audit loss (truncate before insert). Also fixed `ARCHITECTURE.md` status drift. Gates: `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `SKIP_DB_TESTS=1 pnpm test` (593 passed, 21 skipped), `pnpm build`, Mini App bundle budget (168876 B, +8.02% vs baseline, under 10%), `.codex/kit/diagnose.ps1`, `.claude/kit/diagnose.ps1`, and `git diff --check`. Final status remains 8/10 GOOD with UNREACHABLE_10 reason (a): remaining caps are live Neon/Yandex/e2e and Phase 6+/8 ops scope.

- 2026-05-17: Phase 5 step-perfect-loop closed on `phase/5-matching-scoring` with tag `phase-5-perfect`. Full 5×5 depth, Mode B (memory-injection) for sub-loops within main loops, fresh `Agent()` for main_loop=2 sub_loop=1 (PERFECT_FRESH attempt). Trajectory: MIN 6 → 7 → 8 → 8 (main-1), MIN 7 → fresh main-2 with inline a11y fix. Best MIN=8, all reviewers `blockers=[]` in steady state. Final status: ⚠ UNREACHABLE_10 reason (a) — scaffold-phase scope objectively caps below 10 (Phase 4 precedent). Loop applied 20 distinct fixes across ~35 file edits: 8 correctness (cluster_id race + advisory_xact_lock, Unicode keyword regex, stale cosine, empty-after-normalize guard, recompute_topic_embedding terminal flip, extractJsonObject reorder, listRadarMatches single-statement), 2 security (XSS isSafeExternalUrl + userinfo/IDN), 3 perf (migrations 0009 NULLS LAST + topic_profiles partial idx, 0010 extended idx with created_at DESC), 5 UX (theme tokens, filter-empty state, WebApp.openLink, Russian plural + aria-controls + warning token, ARIA APG arrow-key nav + :focus-visible + aria-live), 3 architecture/docs (cluster-dedup invariant reconciliation, page-beyond-last decision-log, empty-topic terminal-state invariant). Gates: typecheck/lint/prettier green; 586+ tests pass; build green; miniapp bundle within budget. Loop report: `.claude/perfect-loop-runs/2026-05-17-phase-5/REPORT.md`.
