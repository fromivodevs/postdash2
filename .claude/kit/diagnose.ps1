...
if (-not (Test-Path $hooksDir)) {
    Write-Host 'FAIL: .claude/hooks not found'
    exit 1
}
...
if (Test-Path $settingsPath) {
    try {
        ...
        if ($settingsText -match '\{\{PROJECT_DIR\}\}') {
            $results += Add-Result 'settings.json' 'FAIL' 'unreplaced {{PROJECT_DIR}} placeholder'
        } elseif ($settingsText -match '\\\.claude\\hooks\\') {
            $results += Add-Result 'settings.json' 'FAIL' 'hook path uses backslashes; use forward slashes in JSON commands'
        } else {
            $results += Add-Result 'settings.json' 'OK' 'valid JSON'
        }
    } catch {
        $results += Add-Result 'settings.json' 'FAIL' $_.Exception.Message
    }
} else {
    $results += Add-Result 'settings.json' 'FAIL' 'missing settings file'
}
...
Test-Skill-Frontmatter (Join-Path $claudeDir 'skills') 'claude skills'
