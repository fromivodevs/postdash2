...
7. Run platform diagnostics before reporting success.
8. For Codex, use model mapping: opus -> `gpt-5.5`, sonnet -> `gpt-5.4`, haiku -> `gpt-5.4-mini`.
...
12. Merge `.gitattributes` rules for PowerShell/script safety.
13. Append `CLAUDE.md.addition` if missing for Claude target.
14. Append `codex/AGENTS.md.addition` if missing for Codex target.
15. Merge settings from `SETTINGS_TEMPLATE.md`, replacing `<PROJECT_DIR>` with absolute project path using `/`.
16. Run Claude target diagnostic when installed:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\.claude\kit\diagnose.ps1
```

17. Run Codex target diagnostic when installed:
