...
### a) Plan-aware
На старте читает `PLAN.md` (или `ROADMAP.md`, `.codex/plan.md`) проекта,
...
Хук `.codex/hooks/stage-complete-detector.ps1` срабатывает на PostToolUse Edit/Write
когда в PLAN.md/ROADMAP.md/.codex/plan.md появляется новая `- [x]` где была `- [ ]`.
...
3. Сохраняет полный отчёт в `.codex/perfect-loop-runs/<ts>-<stage-slug>/REPORT.md`
...
См. STOP RULES в `.codex/skills/perfect-loop/SKILL.md` — step-perfect-loop наследует их полностью с поправкой на меньший ceiling в step-default.
